...
// Botnet server
//
// Command line: ./tsamvgroup26 <port to be on> 
//
// Author: Kjartan Már Andersen (kjartana17@ru.is)
//
#include <stdio.h>
#include <errno.h>
#include <stdlib.h>
...
// Class for handling conections from servers.
//
// Server(int socket) - socket to send/recieve traffic from
class Server
{
  public:
...
void closeClient(int serverSocket, fd_set *openSockets, int *maxfds)
{
                  ...
                  char *buffer) 
{
  ...
  if (tokens[0].compare("LISTSERVERS"))
  else
  {
...
int main(int argc, char* argv[])
{
    ...
    socklen_t serverLen;
    socklen_t clientLen;
    char buffer[1025];              // buffer for reading from server
    ...
    if(getaddrinfo(addressToConnectTo, portToConnectTo, &hints, &svr) != 0)
    {
        perror("getaddrinfo failed: ");
        exit(0);
    }

    serverSock = socket(svr->ai_family, svr->ai_socktype, svr->ai_protocol);
